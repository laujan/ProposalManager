...
var point = (function () {
    var point = {
        ...
        endpoints: {
            ...
            externalCatalog: "/api/SourcePointCatalogs?external=true",
    ...
    that.init = function (accessToken) {
        ...
        that.controls.addExt.click(function () {
            that.action.addExt();
        });
    ...
    that.action = {
            ...
            that.utility.mode(function () {
                ...
                that.service.recentFiles(function (result) {
                    if (result.status == app.status.succeeded) {
                        ...
                        $(".part.part-title").show();
                        $(".part.part-browse").show();
        ...
        addExt: function () {
            // hide recent files and browse button
            $(".part.part-title").hide();
            $(".part.part-browse").hide();
            that.ui.recentFiles({ data: [] }, function () {
                that.selected = null;
                that.default();
                that.browse.browseFile = true;
                that.browse.init(true);    
            });
        },
        back: function () {
        ...
        save: function (o) {
            ...
            that.utility.mode(function () {
                ...
                else if (that.selected.SourceType === app.sourceTypes.chart) {
                    _dpTypes = app.destinationTypes.chart;
                } else {
                    _dpTypes = app.destinationTypes.image;
                }
    ...
    that.browse = {
        ...
        init: function (isExternal) {
            ...
            } else // get external resources
            {
                ...
                // get catalogs
                that.service.externalCatalog(null, function (result) {
                    if (result.status === app.status.succeeded) {
                        that.browse.popup.processing(false);
                        let catalogs = result.data;
                        let _l = [];

                        $.each(catalogs, function (i, d) {
                            _l.push({ id: d.DocumentId, name: decodeURI(d.Name), type: "external" });
                        });
                        that.browse.display({ data: _l });

                    }
                    else {
                        that.document.error({ title: "Error retrieving external catalogs." });
                    }
                });
            }
        },
        ...
        display: function (options) {
            ...
            $.each(options.data, function (i, d) {
                ...
                if (d.type == "site") {
                    ...
                    _h = '<li class="i-site"' + '" data-type="external" data-id="' + d.id + '">' + d.name + '</li>';
                }
                that.controls.popupBrowseList.append(_h);
            });
        ...
        select: function (elem) {
            ...
            if (_t == "site") {
                ...
                that.browse.popup.nav();
                that.browse.popup.processing(true);
                that.browse.sites({ type: "site", siteId: $(elem).data("id"), siteUrl: $(elem).data("siteurl") });
            }
            else if (_t == "library") {
                ...
                that.browse.path.push({ type: "external", id: $(elem).data("id") });
                that.browse.popup.processing(true);
                that.browse.items({ type: "external", documentId: $(elem).data("id") });
            }
            else {
                if (that.browse.browseFile) {
                    that.browse.popup.processing(true);
    ...
    that.format = {
        convert: function (options) {
            var _t = $.trim(options.value != null ? options.value : ""),
                ...
                _x = options.decimal;
            $.each(_f, function (_a, _b) {
                let name = _b.CustomFormat ? _b.CustomFormat.Name : _b.Name;
                if (!_b.IsDeleted) {
                    if (name == "ConvertToHundreds") {
                    ...
                    else if (name == "ConvertToThousands") {
                        if (that.format.isNumber(_v)) {
                            var _l = that.format.getDecimalLength(_v);
                    ...
                    else if (name == "ConvertToMillions") {
                        if (that.format.isNumber(_v)) {
                            var _l = that.format.getDecimalLength(_v);
                    ...
                    else if (name == "ConvertToBillions") {
                        if (that.format.isNumber(_v)) {
                            var _l = that.format.getDecimalLength(_v);
                    ...
                    else if (name == "ShowNegativesAsPositives") {
                    ...
                    else if (name == "IncludeHundredDescriptor") {
                        if (that.format.isNumber(_v)) {
                            _v = _v + " hundred";
                        }
                    }
                    else if (name == "IncludeThousandDescriptor") {
                        if (that.format.isNumber(_v)) {
                            _v = _v + " thousand";
                        }
                    }
                    else if (name == "IncludeMillionDescriptor") {
                        if (that.format.isNumber(_v)) {
                            _v = _v + " million";
                        }
                    }
                    else if (name == "IncludeBillionDescriptor") {
                        if (that.format.isNumber(_v)) {
                            _v = _v + " billion";
                        }
                    }
                    else if (name == "IncludeDollarSymbol") {
                        if (!that.format.hasDollar(_v)) {
                            _v = that.format.addDollar(_v);
                        }
                    }
                    else if (name == "ExcludeDollarSymbol") {
                        if (that.format.hasDollar(_v)) {
                            _v = that.format.removeDollar(_v);
                        }
                    }
                    else if (name == "DateShowLongDateFormat") {
                        if (that.format.isDate(_v)) {
                            var _tt = new Date(_v);
                            _v = _m[_tt.getMonth()] + " " + _tt.getDate() + ", " + _tt.getFullYear();
                        }
                    }
                    else if (name == "DateShowYearOnly") {
                        if (that.format.isDate(_v)) {
                            var _tt = new Date(_v);
                            _v = _tt.getFullYear();
                        }
                    }
                    else if (name == "ConvertNegativeSymbolToParenthesis") {
                        var _h = that.format.hasDollar(_v),
                            _pt = that.format.hasPercent(_v),
                            _hh = _v.toString().indexOf("hundred") > -1,
                            _ht = _v.toString().indexOf("thousand") > -1,
    ...
    that.range = {
        ...
        valueItem: function (controlItems, options, ctx, callback) {
            ...
            var _dsp = options.data[options.index],
                ...
                _cf = _dsp.DestinationPointCustomFormats,
            ...
            if (!_cc) {
            ...
            else if (_sp.SourceType === app.sourceTypes.image) {
                ...
                return ctx.sync().then(function () {
                    ...
                    return ctx.sync().then(function () {
                        var _v = _sp.Value, _c = _src.value !== _v;
    ...
    that.service = {
        ...
        externalCatalog: function (options, callback) {
            that.service.common({ url: that.endpoints.externalCatalog, type: "GET", dataType: "json" }, callback);
        },
    ...
    that.ui = {
        ...
        sources: function (options) {
            ...
            if (options.sourceType == app.sourceTypes.all) {
                ...
                if (_ia > 0) {
                    ...
                    // images
                    $(that.controls.sourceTypeNav[3]).addClass("is-selected");
                    $.each(_da, function (i, d) {
                        if (d.SourceType === app.sourceTypes.image) {
                            _dl.push(d);
                        }
                    });
                }
            ...
            that.controls.sourceTypeNav[3].children[1].innerText = _id;

            $.each(_dl, function (i, d) {
                var _p = that.utility.position(d.Position), _v = d.SourceType == app.sourceTypes.point ? (d.Value ? d.Value : "") : "",
                    _t = d.SourceType == app.sourceTypes.point ? "Source point" : (d.SourceType == app.sourceTypes.table ? "Source table" : d.SourceType == app.sourceTypes.chart ? "Source chart" : "Source image");
                ...
                that.range.values({ data: $.extend([], that.points), tags: _tags, refresh: options.refresh }, function (result) {
                    ...
                    $.each(_d, function (i, d) {
                        if (d.ReferencedSourcePoint.SourceType == app.sourceTypes.point) {
                            ...
                            _ib++;
                        } else if (d.ReferencedSourcePoint.SourceType == app.sourceTypes.table) {
                            _ic++;
                        } else
                            _id++;
                    });
                    ...
                    that.controls.sourceTypeNavMana[3].children[1].innerText = _id;
            ...
            if (options.index < options.data.length) {
                ...
                if (options.index >= that.pagerSize * (that.pagerIndex - 1) && options.index < that.pagerSize * that.pagerIndex) {
                    var _spv = _item.Value && _item.Value != null ? _item.Value : "",
                        ...
                        _fv = _sourceType == app.sourceTypes.point ? that.format.convert({ value: _spv, formats: _dsp.DestinationPointCustomFormats, decimal: _dsp.DecimalPlace }) : _v,
                        _ff = [],
                    ...
                    $.each(_dsp.DestinationPointCustomFormats != null ? _dsp.DestinationPointCustomFormats : [], function (_x, _y) { _ff.push(_y.CustomFormat.DisplayName); });
        ...
        customFormat: function (options, callback) {
            ...
            if (options.selected && options.selected != null) {
                $.each(options.selected.DestinationPointCustomFormats, function (_x, _y) {
                    _si.push(_y.CustomFormat.Id);
                    _sn.push(_y.CustomFormat.Name);
                    _sd.push(_y.CustomFormat.DisplayName);
                });
