...
import { OpportunityListCompact } from './OpportunityListCompact';
            ...
            if (response.ok) {
                ...
                if (data.ItemsList.length > 0) {
                    for (let i = 0; i < data.ItemsList.length; i++) {
                        ...
                        newItem.createTeamDisable = true; //item.dealType !== null && item.dealType.id !== null ? false : true;
                ...
                //filteredItems = itemslist.filter(itm => itm.status.toLowerCase === "creating" || itm.status.toLowerCase === "inprogress");
        ...
        if (this.state.items.length > 0) {
            ...
            this.createTeam(item.opportunity)
                .then((res, err) => {
                    ...
                    this.getOpportunity(item.id)
                        .then(res => {
                            let processList = res.template.processes;
